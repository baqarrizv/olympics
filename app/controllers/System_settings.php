...
class system_settings extends MY_Controller
{
    ...
    function getWarehouses()
    {
        $this->load->library('datatables');
        $this->datatables
            ->select("warehouses.code, warehouses.name, c.city_name, warehouses.email, warehouses.phone, warehouses.contact_person_name as person, warehouses.contact_person_designation as designation, warehouses.contact_person_email as person_email")
            ->from("warehouses")
            //->where("parent_id", '0')
    ...
    {
        // $s = $this->db->select("c.city_name, wh.name, fin_locations.loc_code, fin_locations.location_name, fin_locations.measure_unit, fin_locations.capacity, SUM(move.qty)")
        //     ->from("fin_locations")
        //     //->where("parent_id", '0')
    ...
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang("code"), 'trim|is_unique[fin_locations.loc_code]|required');
        $this->form_validation->set_rules('name', lang("name"), 'required');

        if ($this->form_validation->run() == true) {
            
            $data = array('loc_code' => $this->input->post('code'),
                'warehouse_id' => $this->input->post('warehouse'),
                'location_name' => $this->input->post('name'),
                'measure_unit' => $this->input->post('uom'),
                'capacity' => $this->input->post('capacity'),
            );

        } 
        if ($this->form_validation->run() == true && $this->settings_model->addSilos($data)) {
            $this->session->set_flashdata('message', lang("silos added"));
            redirect("system_settings/silos");
        } else {

            $this->db->select('city_code,city_name')
             ->from('sma_city');
            $this->data['city'] = $this->db->get()->result_array();
            $this->db->select('*');
            $this->db->from('sma_warehouses');
            $this->db->where('parent_id', '0');
            $this->data['warehouses'] = $this->db->get()->result_array();
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_silos', $this->data);
        }
    }
    ...
    function add_warehouse()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'contact_person_name' => $this->input->post('person_name'),
                'contact_person_designation' => $this->input->post('person_designation'),
                'contact_person_email' => $this->input->post('person_email'),
        ...
        if ($this->form_validation->run() == true && $this->settings_model->addWarehouse($data)) {
            ...
            $this->db->select('id,city_code,city_name');
            $this->data['city'] = $this->db->get('city')->result_array();
    ...
    function silos()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('silos')));
        $meta = array('page_title' => lang('Silos'), 'bc' => $bc);
        $this->page_construct('settings/silos', $meta, $this->data);
    }
