...
class Site extends CI_Model
{
    ...
    public function getDensityChart()
    {
        $q = $this->db->get('density_chart');
        if ($q->num_rows() > 0) {
    ...
    {
        $this->db->select("warehouses.id, warehouses.code, warehouses.name, c.city_name, warehouses.email, warehouses.phone, warehouses.contact_person_name as person, warehouses.contact_person_designation as designation, warehouses.contact_person_email as person_email")
            ->join("city c", 'c.id=warehouses.city_id', 'left');
    ...
    public function calculate_reorder_level($location, $stock_id)
    {
        $sql = "SELECT stock.*, loc.location_name, loc.email
            FROM sma_fin_loc_stock stock,
                sma_fin_locations loc
    ...
    function getSilosByLocation($id)
    {
        $this->db->select('sma_fin_locations.*, FORMAT(SUM(sma_fin_stock_moves.qty), 2) as qty')
        ->join('sma_fin_stock_moves', 'sma_fin_stock_moves.loc_code=sma_fin_locations.loc_code', 'left')
        ->group_by('sma_fin_stock_moves.loc_code');
        $q = $this->db->get_where('sma_fin_locations', array('sma_fin_locations.warehouse_id' => $id));
    ...
    function getSaleData()
    {
        ...
        $q = $this->db->get_where('transaction_logs', array('transaction_logs.trans_type' => 'Sales'));
    ...
    {
         $CI =& get_instance();
         return $CI->load->model($name);
         
    }

    public function add_stock_adjustment_item($adj_id, $stock_id, $location, $date_, $reference,
    $quantity, $standard_cost, $memo_)
    {
       
        $mb_flag = $this->get_mb_flag($stock_id);

        if ($this->is_fixed_asset($mb_flag)) {
            $sql = "UPDATE sma_fin_stock_master SET inactive=1
                WHERE stock_id=".$stock_id;

            $this->db->query($sql);
        }
        $this->import_model('Transfers_model');
        $this->import_model('Purchases_model');

        $stock_move_id = $this->Transfers_model->add_stock_move(17, $stock_id, $adj_id, $location,
        ...
        if ($this->is_fixed_asset($mb_flag)) {
            ...
            $row = $this->get_fixed_asset_move($stock_id, 17);
            $inv_value = $row['price'] * $quantity;
            $adj_value = (-($row['price']) + $standard_cost) * $quantity;
            
        }

        if ($standard_cost > 0 || $this->is_fixed_asset($mb_flag))
        {

            $stock_gl_codes = $this->Purchases_model->get_stock_gl_code($stock_id);

            $gl_data = array(
                        'type' => 17,
                        'type_no' => $adj_id,
                        'tran_date' => $date_,
                        'account' => $stock_gl_codes['adjustment_account'],
                        'memo_' => $memo_,
                        'amount' => $adj_value,
                        'dimension_id' => $stock_gl_codes['dimension_id'],
                        'dimension2_id' => 0, 
            );
            
            $this->db->insert('sma_fin_gl_trans', $gl_data);

            $gl_dt = array(
                        'type' => 17,
                        'type_no' => $adj_id,
                        'tran_date' => $date_,
                        'account' => $stock_gl_codes['inventory_account'],
                        'memo_' => $memo_,
                        'amount' => $inv_value,
                        'dimension_id' => 0,
                        'dimension2_id' => 0,
            ...
            $this->db->insert('sma_fin_gl_trans', $gl_dt);
        ...
        //     add_gl_trans_std_cost(17, $adj_id, $date_, $grn_act, 0, 0, $memo_, ($standard_cost * -($quantity)));
        // }
    }

    public function get_fixed_asset_move($stock_id, $tran_type) {

      $sql = "SELECT * FROM sma_fin_stock_moves 
            WHERE stock_id=".$stock_id."
                AND type=".$tran_type;
    ...
    public function get_mb_flag($stock_id)
    {
        $sql = "SELECT mb_flag FROM sma_fin_stock_master
            WHERE stock_id = ".$stock_id;
    ...
    public function is_fixed_asset($mb_flag)
    {
        return ($mb_flag == 'F');
    }

   public function getWarehouseByID($id) {
        $q = $this->db->get_where('fin_locations', array('loc_code' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
